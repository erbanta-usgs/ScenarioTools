...
namespace USGS.Puma.Modflow
{
    public class DisDataReader
    {
        ...
        public DisDataReader(int fileUnit)
            : this()
        {
            _FileUnit = fileUnit;
        }
        ...
        public DisFileData Read(ModflowNameData nameData)
        {
            ...
            // File unit is required for interpreting fixed-format array control lines
            _FileUnit = nfItems[0].FileUnit;

            // Open file and read data
            using (StreamReader reader = new StreamReader(disFilename))
            {
                mdaReader = new ModflowDataArrayReader<float>(reader, nameData);
                mdaReader.SetFileUnit(_FileUnit);
                string line = null;
                ...
                // Add units by mlago on 7/12/2014. 
                // Previously disFileData.TimeUnit and disFileData.LengthUnit were always zero.
                if (timeUnit >= 0 && timeUnit <= 5) disFileData.TimeUnit = timeUnit;
                if (lengthUnit >= 0 && lengthUnit <= 3) disFileData.LengthUnit = lengthUnit;
