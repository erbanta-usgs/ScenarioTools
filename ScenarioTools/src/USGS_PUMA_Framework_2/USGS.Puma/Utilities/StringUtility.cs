...
namespace USGS.Puma.Utilities
{
    ...
    public class StringUtility
    {
        ...
        /// <summary>
        /// Returns a string array from a fixed-format array-reader control line.
        /// </summary>
        /// <param name="sourceLine"></param>
        /// <returns></returns>
        public static string[] SplitFixedFormatControlLine(string sourceLine)
        {
            // This function returns a string array from a fixed-format array-reader control line.
            // The control line is expected to contain: LOCAT CNSTNT FMTIN IPRN
            List<string> list = new List<string>();
            int start = 0;
            int defaultFieldWidth = 10;
            int fieldWidth;
            for (int i = 0; i < 4; i++)
            {
                // Determine width of current field
                ...
                // Check remaining string and return if the remaining length < fieldWidth
                int endPosition = start + fieldWidth;
                if (endPosition > sourceLine.Length) endPosition = sourceLine.Length - 1; //return list.ToArray();

                // Add field contents to list
                list.Add(sourceLine.Substring(start, fieldWidth));

                // Advance to next field
                start = start + fieldWidth;
            }
