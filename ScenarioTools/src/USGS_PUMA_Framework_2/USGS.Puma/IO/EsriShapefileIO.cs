...
namespace USGS.Puma.IO
{
    ...
    public static class EsriShapefileIO
    {
        ...
        /// <param name="filename"></param>
        /// <returns></returns>
        public static FeatureCollection Import(string filename, BackgroundWorker backgroundWorker)
        {
            FeatureCollection featureList = new FeatureCollection();
            ShapefileDataReader shpDataReader = null;
            try
            {
                ...
                if (shpDataReader.RecordCount > 0)
                {
                    ...
                    // Variables for reporting of progress through BackgroundWorker
                    int maxProgress = 100;
                    int reportProgressInc = shpDataReader.RecordCount / maxProgress;
                    if (reportProgressInc < 1)
                    {
                        reportProgressInc = 1;
                    }
                    int progress = 0;
                    ...
                    while (true)
                    {
                        ...
                        if (moreRecords)
                        {
                            ...
                            {
                                progressCount++;
                                if (progressCount == reportProgressInc)
                                {
                                    if (progress < 100)
                                    {
                                        progress++;
