...
namespace USGS.Puma.FiniteDifference
{
    public class ModflowGrid : IModflowGrid
    {
        ...
        public static IModflowGrid Create(ControlFileDataImage dataImage)
        {
            ...
            string blockGeoReference = "georeference";
            ...
            double xOffset = 0.0;
            double yOffset = 0.0;
            if (dataImage.ContainsBlock(blockGeoReference))
            {
                ControlFileBlock georefBlock = dataImage[blockGeoReference];
                angle = georefBlock["rotation_angle"].GetValueAsDouble();
                xOffset = georefBlock["x_offset"].GetValueAsDouble();
                yOffset = georefBlock["y_offset"].GetValueAsDouble();
            }
