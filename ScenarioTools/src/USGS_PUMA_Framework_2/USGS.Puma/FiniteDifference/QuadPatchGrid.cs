...
namespace USGS.Puma.FiniteDifference
{
    public class QuadPatchGrid : IQuadPatchGrid, IDataObject
    {
        ...
        public static IQuadPatchGrid Create(ControlFileDataImage dataImage)
        {
            ...
            if (quadpatchBlock.Contains("modflow_grid"))
            {
                string mfGridLabel = quadpatchBlock["modflow_grid"].GetValueAsText();
                blockModflowGrid = blockModflowGrid + ":" + mfGridLabel;
            }
            ...
            double rotationAngle = modflowGridBlock["rotation_angle"].GetValueAsDouble();
            ...
            {
                style = quadpatchBlock["smoothing"].GetValueAsText();
            }

            QuadPatchSmoothingType smoothingType;
            if (style == "none")
            ...
            {
                throw new Exception("Invalid smoothing type was read from control file.");
            }

            int maxLevelDifference = 1;
            if (quadpatchBlock.Contains("maximum_level_difference"))
            {
                maxLevelDifference = quadpatchBlock["maximum_level_difference"].GetValueAsInteger();
            }

            // Create instance of QuadPatch
