...
namespace ModflowOutputViewer
{
    ...
    public class LayerAnalysis
    {
        ...
        public Array2d<float> CreateAnalysisArray(Array2d<float> currentLayerArray, Array2d<float> referenceLayerArray, List<float> excludedValuesCurrentLayer, List<float> excludedValuesReferenceLayer, float analysisArrayNoDataValue)
        {
            if (currentLayerArray == null)
                return null;
            if (referenceLayerArray == null)
                return null;
            if (currentLayerArray.RowCount != referenceLayerArray.RowCount)
                throw new ArgumentException("The current layer array and the reference layer array must be the same size.");
            if (currentLayerArray.ColumnCount != referenceLayerArray.ColumnCount)
                throw new ArgumentException("The current layer array and the reference layer array must be the same size.");
            
            Array2d<float> buffer = new Array2d<float>(currentLayerArray.RowCount, currentLayerArray.ColumnCount);
            ...
            switch (AnalysisType)
            {
                case LayerAnalysisType.ReferenceLayerValues:
                    for (int row = 1; row <= buffer.RowCount; row++)
                    {
                        ...
                        {
                            r = referenceLayerArray[row, column];
                            if (IsExcludedValue(excludedValuesReferenceLayer, r))
                            { buffer[row, column] = analysisArrayNoDataValue; }
                            else
                            { buffer[row, column] = r; }
                        }
                    }
        ...
        public void UpdateRenderer(ColorRampRenderer renderer, Array2d<float> analysisArray, List<float> excludedValues)
        {
            if (renderer == null)
            { return; }
            ...
            UpdateRenderer(renderer, excludedValues);

        }
        public void UpdateRenderer(ColorRampRenderer renderer, List<float> excludedValues)
        {
            if (renderer == null)
            { return; }
            ...
            // If excludedValues is not null, update the excluded values
            if (excludedValues != null)
            {
                renderer.ExcludedValues.Clear();
                foreach (float excludedValue in excludedValues)
