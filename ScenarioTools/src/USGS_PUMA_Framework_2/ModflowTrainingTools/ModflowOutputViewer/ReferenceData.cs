...
namespace ModflowOutputViewer
{
    ...
    public class ReferenceData : IDisposable
    {
        ...
        private BinaryLayerReader _FileReader;
        ...
        public bool OpenFile(string filename, float hNoFlo, float hDry)
        {
            ...
            if (reader.Valid)
            {
                ...
                {
                    LayerDataRecordHeader header = reader.ReadRecordHeader(0);
        ...
        public LayerDataRecord<float> GetLayerDataRecord(LayerDataRecord<float> currentLayerDataRecord)
        {
            if (FileReader == null)
            { return null; }
            ...
            //currentRecordKey = BuildRecordKey(cr.StressPeriod, cr.TimeStep, cr.Layer, dataType);
            key = "";
            ...
            rec = FileReader.GetRecordAsSingle(0);
            dataType = rec.Text.Trim().ToUpper();
            key = BuildRecordKey(stressPeriod, timeStep, modelLayer, dataType);

            recIndex = FileReader.FindRecordIndex(key);
            rec = null;
            if (recIndex > -1)
            {
                rec = FileReader.GetRecordAsSingle(recIndex);
            }

            // return reference layer data record
            return rec;

            //if (FileReader.OutputPrecision== OutputPrecisionType.Single)
            //{
            //    rec = FileReader.GetRecordAsSingle(0);
            //    dataType = rec.Text.Trim().ToUpper();
            //    key = BuildRecordKey(stressPeriod, timeStep, modelLayer, dataType);
            ...
            //else
            //{
            //    LayerDataRecord<double> recDouble = FileReader.GetRecord(0) as LayerDataRecord<double>;
            //    dataType = recDouble.Text.Trim().ToUpper();
            //    key = BuildRecordKey(stressPeriod, timeStep, modelLayer, dataType);
            ...
            //    if (recIndex > -1)
            //    {
            //        recDouble = FileReader.GetRecord(recIndex) as LayerDataRecord<double>;
            //        rec = recDouble.CopyAsSingle();
            //    }
